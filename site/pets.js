...
(function (root) {
  ...
  const SHELF_OF = (id) => (HATS[id] ? "hat" : FACE[id] ? "face" : EARS[id] ? "ears" : null);
  ...
  function wear(THREE, pet, sp, id, shelf) {
    ...
    for (const c of head.children.slice()) if (c.userData.outfit && c.userData.shelf === target) head.remove(c);
    if (!id) return null;
  ...
  root.Perchlings = { SPECIES, MOODS, POSES, STYLE, HATS, FACE, EARS, makePet, makeOutfit, wear, setMood, setPose, addLights, setupRenderer };
