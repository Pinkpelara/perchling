// Perchlings: the pets, as three.js geometry. Single source of truth for the look.
// Used by web/look-test.html (demo), tools/sprites/sprite.html (frame renderer), and later the app.
// Depends on a global THREE (r128 UMD).
(function (root) {
  const SPECIES = [
    // style: "cute" = baby proportions (big low eyes, blush). "calm" = a little older. "cool" = grown, half-lidded, no blush.
    { id: "antenna", label: "Teal",  style: "cute", body: "#2FB3A3", belly: "#BFE8E0", shade: "#1F7F73", extra: "#7EDCD0" },
    { id: "ears",    label: "Pink",  style: "cute", body: "#EE8FA4", belly: "#F9D0D9", shade: "#C46A7E", extra: "#F3ADBD" },
    { id: "leaf",    label: "Green", style: "calm", body: "#58A64E", belly: "#C6E0B4", shade: "#356F2A", extra: "#7CC46A" },
    { id: "horns",   label: "Gold",  style: "cool", body: "#E0862A", belly: "#F4D2A0", shade: "#A85F16", extra: "#F2B85A" },
  ];
  const MOODS = ["happy", "surprised", "sleepy", "sulky"];
  const POSES = ["idle", "blink", "walk1", "walk2", "squash", "stretch", "sit", "lie", "wave1", "wave2", "study", "work", "game", "eat1", "eat2", "dab", "flex"];
  const EYE = "#1E1B24", MOUTH = "#3B2733", BLUSH = "#F58EA6", TONGUE = "#F27C8F", BROW = "#2A2230";
  const STYLE = {
    cute: { eye: 1.0,  eyeY: 0.02, blush: 0.8,  browAmp: 1.0, browY: 0.20, mouth: 1.0,  lids: false, bodyH: 0.44, headR: 0.56, faceY: -0.06 },
    calm: { eye: 0.88, eyeY: 0.05, blush: 0.45, browAmp: 0.8, browY: 0.20, mouth: 0.85, lids: false, bodyH: 0.46, headR: 0.55, faceY: -0.04 },
    cool: { eye: 0.76, eyeY: 0.09, blush: 0,    browAmp: 0.45, browY: 0.17, mouth: 0.7,  lids: true,  bodyH: 0.5,  headR: 0.52, faceY: -0.02 },
  };

  function materials(THREE, sp) {
    const vinyl = (hex) => new THREE.MeshPhysicalMaterial({ color: new THREE.Color(hex), roughness: 0.3, metalness: 0.03, clearcoat: 0.9, clearcoatRoughness: 0.2 });
    return { body: vinyl(sp.body), shade: vinyl(sp.shade), extra: vinyl(sp.extra), belly: vinyl(sp.belly) };
  }
  function matte(THREE, hex, opts) {
    return new THREE.MeshStandardMaterial(Object.assign({ color: new THREE.Color(hex), roughness: 0.55, metalness: 0.02 }, opts || {}));
  }
  function ball(THREE, mat, pos, scale, segs) {
    const m = new THREE.Mesh(new THREE.SphereGeometry(1, segs || 48, segs || 48), mat);
    m.position.set(pos[0], pos[1], pos[2]);
    if (typeof scale === "number") m.scale.setScalar(scale); else m.scale.set(scale[0], scale[1], scale[2]);
    m.castShadow = true; m.receiveShadow = true; return m;
  }
  function arc(THREE, mat, radius, tube, pos, rotZ, span) {
    const m = new THREE.Mesh(new THREE.TorusGeometry(radius, tube, 10, 28, span == null ? Math.PI : span), mat);
    m.position.set(pos[0], pos[1], pos[2]); m.rotation.z = rotZ || 0; return m;
  }

  function makePet(THREE, sp) {
    const st = STYLE[sp.style] || STYLE.cute;
    const g = new THREE.Group();
    const M = materials(THREE, sp);
    const wide = sp.id === "ears";

    const bodyScale = wide ? [0.56, st.bodyH, 0.5] : [0.48, st.bodyH, 0.46];
    const bodyRoot = new THREE.Group(); g.add(bodyRoot);
    bodyRoot.add(ball(THREE, M.body, [0, -0.2, 0], bodyScale));
    bodyRoot.add(ball(THREE, M.belly, [0, -0.25, bodyScale[2] * 0.62], [0.3 * (wide ? 1.15 : 1), 0.24, 0.18]));
    const feet = [ball(THREE, M.shade, [-0.16, -0.66, 0.08], [0.15, 0.085, 0.17], 24), ball(THREE, M.shade, [0.16, -0.66, 0.08], [0.15, 0.085, 0.17], 24)];
    feet.forEach((f) => bodyRoot.add(f));
    const arms = [ball(THREE, M.body, [-(bodyScale[0] - 0.05), -0.16, 0.1], [0.12, 0.13, 0.12], 24), ball(THREE, M.body, [bodyScale[0] - 0.05, -0.16, 0.1], [0.12, 0.13, 0.12], 24)];
    arms.forEach((a) => bodyRoot.add(a));

    const headY = 0.42 + (st.bodyH - 0.44) * 0.8;
    const head = new THREE.Group(); head.position.set(0, headY, 0); g.add(head);
    const headR = st.headR;
    head.add(ball(THREE, M.body, [0, 0, 0], wide ? [headR * 1.07, headR * 0.96, headR * 1.04] : headR));

    const face = new THREE.Group(); face.position.set(0, st.faceY, headR * 0.9); head.add(face);
    const eyes = new THREE.Group(); face.add(eyes); const eyeParts = [];
    for (const sx of [-1, 1]) {
      const eye = new THREE.Group(); eye.position.set(sx * 0.17, st.eyeY, 0);
      eye.add(ball(THREE, matte(THREE, EYE, { roughness: 0.25 }), [0, 0, 0], [0.105 * st.eye, 0.135 * st.eye, 0.07], 24));
      eye.add(ball(THREE, matte(THREE, "#ffffff", { emissive: 0xffffff, emissiveIntensity: 0.7 }), [-0.03 * sx * st.eye, 0.05 * st.eye, 0.065], 0.034 * st.eye, 12));
      eye.add(ball(THREE, matte(THREE, "#ffffff", { emissive: 0xffffff, emissiveIntensity: 0.7 }), [0.035 * sx * st.eye, -0.045 * st.eye, 0.06], 0.016 * st.eye, 10));
      if (st.lids) eye.add(ball(THREE, M.body, [0, 0.075 * st.eye, 0.02], [0.125 * st.eye, 0.075 * st.eye, 0.07], 20));  // half-lidded
      eyes.add(eye); eyeParts.push(eye);
    }
    const sleepyLines = new THREE.Group(); face.add(sleepyLines);
    for (const sx of [-1, 1]) {
      const m = new THREE.Mesh(new THREE.BoxGeometry(0.13 * st.eye, 0.024, 0.02), matte(THREE, EYE));
      m.position.set(sx * 0.17, st.eyeY - 0.02, 0.02); m.rotation.z = sx * -0.18; sleepyLines.add(m);
    }
    const brows = [];
    for (const sx of [-1, 1]) {
      const b = new THREE.Mesh(new THREE.BoxGeometry(0.11, st.lids ? 0.018 : 0.022, 0.02), matte(THREE, BROW));
      b.position.set(sx * 0.17, st.browY, 0.0); face.add(b); brows.push(b);
    }
    const blush = new THREE.Group(); face.add(blush);
    if (st.blush > 0) for (const sx of [-1, 1]) blush.add(ball(THREE, matte(THREE, BLUSH, { transparent: true, opacity: st.blush }), [sx * 0.31, -0.1, -0.02], [0.085, 0.05, 0.03], 16));

    const mk = st.mouth;
    const smile = arc(THREE, matte(THREE, MOUTH), 0.075 * mk, 0.015, [0, -0.13, 0.02], Math.PI, Math.PI);
    const frown = arc(THREE, matte(THREE, MOUTH), 0.06 * mk, 0.015, [0, -0.18, 0.02], 0, Math.PI * 0.9);
    const open = new THREE.Group(); open.position.set(0, -0.15, 0.02); face.add(open);
    open.add(ball(THREE, matte(THREE, MOUTH, { roughness: 0.3 }), [0, 0, 0], [0.065 * mk, 0.055 * mk, 0.03], 20));
    open.add(ball(THREE, matte(THREE, TONGUE, { roughness: 0.35 }), [0, -0.025 * mk, 0.012], [0.038 * mk, 0.022 * mk, 0.02], 16));
    const oh = ball(THREE, matte(THREE, MOUTH, { roughness: 0.3 }), [0, -0.15, 0.02], [0.04, 0.05, 0.03], 20); face.add(oh);
    const flat = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.014, 0.02), matte(THREE, MOUTH)); flat.position.set(0, -0.14, 0.02); face.add(flat);
    face.add(smile); face.add(frown);

    if (sp.id === "antenna") {
      const stalk = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.04, 0.34, 14), M.shade);
      stalk.position.set(0, headR + 0.12, 0); stalk.castShadow = true; head.add(stalk);
      head.add(ball(THREE, M.extra, [0, headR + 0.34, 0], 0.13, 24));
    }
    if (sp.id === "ears") { head.add(ball(THREE, M.body, [-0.64, 0.12, 0.02], [0.2, 0.16, 0.14])); head.add(ball(THREE, M.body, [0.64, 0.12, 0.02], [0.2, 0.16, 0.14])); }
    if (sp.id === "leaf") {
      const a = ball(THREE, M.extra, [-0.1, headR + 0.06, 0.0], [0.26, 0.14, 0.2]); a.rotation.set(0.5, 0, -0.45); head.add(a);
      const b = ball(THREE, M.shade, [0.13, headR + 0.1, -0.04], [0.22, 0.1, 0.15]); b.rotation.set(0.4, 0, 0.5); head.add(b);
    }
    if (sp.id === "horns") {
      for (const sx of [-1, 1]) {
        const h = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.36, 6), M.shade);
        h.position.set(sx * 0.33, headR - 0.04, 0); h.rotation.z = sx * -0.55; h.castShadow = true; head.add(h);
      }
    }

    g.userData.style = sp.style;
    // Props for the "together" activities. Built once, hidden, shown by setPose. They sit on the floor in front of a sitting pet.
    const props = {}; const propRoot = new THREE.Group(); g.add(propRoot);
    const dark = matte(THREE, "#3A3550", { roughness: 0.5 }), cream = matte(THREE, "#F4F1EA", { roughness: 0.7 }), grey = matte(THREE, "#8E89A8", { roughness: 0.5 });
    {
      const laptop = new THREE.Group();
      const base = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.03, 0.34), dark); base.position.set(0, -0.72, 0.62); laptop.add(base);
      const lid = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.34, 0.03), dark); lid.position.set(0, -0.55, 0.47); lid.rotation.x = -0.25; laptop.add(lid);
      const screen = new THREE.Mesh(new THREE.PlaneGeometry(0.44, 0.28), matte(THREE, "#DDE8FF", { emissive: 0xBFD4FF, emissiveIntensity: 0.6, roughness: 0.9 }));
      screen.position.set(0, -0.55, 0.487); screen.rotation.x = -0.25; laptop.add(screen);
      laptop.scale.set(1.25, 1.25, 1.15); laptop.position.set(0, 0.16, -0.06);
      props.laptop = laptop;
      const book = new THREE.Group();
      const cover = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.06, 0.3), matte(THREE, "#E64C6A", { roughness: 0.6 })); cover.position.set(-0.44, -0.71, 0.55); cover.rotation.y = 0.3; book.add(cover);
      const pages = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.04, 0.28), cream); pages.position.set(-0.44, -0.71, 0.55); pages.rotation.y = 0.3; book.add(pages);
      props.book = book;
      const mug = new THREE.Group();
      const cup = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.065, 0.15, 20), cream); cup.position.set(0.44, -0.67, 0.55); mug.add(cup);
      const handle = new THREE.Mesh(new THREE.TorusGeometry(0.045, 0.014, 8, 20, Math.PI), cream); handle.position.set(0.515, -0.67, 0.55); handle.rotation.z = -Math.PI / 2; mug.add(handle);
      const coffee = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.01, 20), matte(THREE, "#5B3A22", { roughness: 0.3 })); coffee.position.set(0.44, -0.6, 0.55); mug.add(coffee);
      props.mug = mug;
      const pad = new THREE.Group();
      const body = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.07, 0.14), grey); pad.add(body);
      for (const sx of [-1, 1]) { const grip = ball(THREE, grey, [sx * 0.15, -0.02, 0.02], [0.06, 0.045, 0.08], 14); pad.add(grip); }
      [["#E64C6A", 0.1, 0.0], ["#48B7E8", 0.14, -0.04], ["#FFD166", 0.06, -0.04], ["#5FA85A", 0.1, -0.08]].forEach(([c, x, z]) => pad.add(ball(THREE, matte(THREE, c, { roughness: 0.4 }), [x, 0.04, z], 0.014, 8)));
      const stick = ball(THREE, matte(THREE, "#6B6685"), [-0.1, 0.05, -0.02], 0.022, 10); pad.add(stick);
      pad.position.set(0, -0.28, 0.5); pad.rotation.x = -0.55; pad.scale.setScalar(1.35);
      props.pad = pad;
      const bowl = new THREE.Group();
      const dish = ball(THREE, cream, [0, -0.64, 0.62], [0.27, 0.13, 0.27], 24); bowl.add(dish);
      const food = ball(THREE, matte(THREE, "#F2C46B", { roughness: 0.8 }), [0, -0.56, 0.62], [0.22, 0.09, 0.22], 20); bowl.add(food);
      for (const [x, z] of [[-0.09, 0.06], [0.07, -0.04], [0.02, 0.1], [-0.03, -0.09]]) bowl.add(ball(THREE, matte(THREE, "#E64C6A", { roughness: 0.6 }), [x, -0.49, 0.62 + z], 0.04, 10));
      props.bowl = bowl;
      for (const k in props) { props[k].visible = false; propRoot.add(props[k]); }
    }
    g.userData.parts = { head, face, eyes, eyeParts, sleepyLines, brows, blush, smile, frown, open, oh, flat, feet, arms, bodyRoot, props };
    g.userData.armX = bodyScale[0] - 0.05;
    setMood(g, "happy");
    return g;
  }

  // ---- closet -------------------------------------------------------------
  // Items are built in "head space": origin at the point on top of the head, y up,
  // sized for a head radius of 0.55 and scaled to fit each pet. They hang off the
  // head group so every pose moves them too. Names and shelves live in app/closet.json.
  const HAT_SEAT = {   // where a hat sits so the pet's own feature stays in view
    antenna: { tilt: 0.46, y: -0.02 },   // to one side, the antenna stays clear
    ears:    { tilt: 0.0,  y: -0.03 },
    leaf:    { tilt: -0.56, y: 0.0 },    // to the other side, the leaf stays clear
    horns:   { tilt: 0.0,  y: 0.0 },
  };
  const HATS = {
    beanie(THREE) {
      const g = new THREE.Group(), wool = matte(THREE, "#5B57D6", { roughness: 0.85 }), cream = matte(THREE, "#F6EFE3", { roughness: 0.9 });
      g.add(ball(THREE, wool, [0, 0.03, 0], [0.4, 0.8 * 0.4, 0.4], 32));
      const brim = new THREE.Mesh(new THREE.TorusGeometry(0.385, 0.055, 12, 40), cream); brim.rotation.x = Math.PI / 2; brim.position.y = -0.12; g.add(brim);
      g.add(ball(THREE, cream, [0, 0.4, 0], 0.1, 20));
      return g;
    },
    crown(THREE) {
      const g = new THREE.Group(), gold = new THREE.MeshStandardMaterial({ color: new THREE.Color("#F5C242"), roughness: 0.3, metalness: 0.6, side: THREE.DoubleSide });
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.33, 0.22, 32, 1, true), gold); band.position.y = -0.02; g.add(band);
      for (let i = 0; i < 6; i++) {
        const a = i / 6 * Math.PI * 2, pt = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.14, 8), gold);
        pt.position.set(Math.sin(a) * 0.34, 0.16, Math.cos(a) * 0.34); g.add(pt);
      }
      const gems = ["#E64C6A", "#48B7E8", "#E64C6A"];
      gems.forEach((c, i) => g.add(ball(THREE, matte(THREE, c, { roughness: 0.2 }), [(i - 1) * 0.16, 0.0, 0.345], 0.04, 12)));
      return g;
    },
    party(THREE) {
      const g = new THREE.Group();
      const cone = new THREE.Mesh(new THREE.ConeGeometry(0.24, 0.5, 32), matte(THREE, "#6C5CE7", { roughness: 0.5 })); cone.position.y = 0.2; g.add(cone);
      g.add(ball(THREE, matte(THREE, "#FFD166", { roughness: 0.8 }), [0, 0.47, 0], 0.07, 16));
      return g;
    },
    chef(THREE) {
      const g = new THREE.Group(), cloth = matte(THREE, "#F4F1EA", { roughness: 0.75 });
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.34, 0.16, 32), cloth); band.position.y = -0.04; g.add(band);
      g.add(ball(THREE, cloth, [0, 0.22, 0], 0.3, 28));
      g.add(ball(THREE, cloth, [-0.2, 0.16, 0.04], 0.22, 24));
      g.add(ball(THREE, cloth, [0.2, 0.16, 0.04], 0.22, 24));
      return g;
    },
    mushroom(THREE) {
      const g = new THREE.Group(), cap = matte(THREE, "#E24B3F", { roughness: 0.45 }), dot = matte(THREE, "#FFF7EC", { roughness: 0.6 });
      const R = 0.6, ry = 0.58;
      g.add(ball(THREE, cap, [0, 0.02, 0], [R, R * ry, R], 40));
      for (const [x, y, z] of [[0, 0.9, 0.4], [0.7, 0.6, 0.5], [-0.65, 0.55, 0.55], [0.55, 0.45, -0.7], [-0.6, 0.5, -0.6], [0.05, 0.5, -0.85]]) {
        const n = new THREE.Vector3(x, y, z).normalize();
        g.add(ball(THREE, dot, [n.x * R * 1.02, 0.02 + n.y * R * ry * 1.02, n.z * R * 1.02], [0.09, 0.06, 0.09], 12));
      }
      return g;
    },
    cap(THREE) {   // a snapback, worn a little back
      const g = new THREE.Group(), cloth = matte(THREE, "#1E1B24", { roughness: 0.8 }), accent = matte(THREE, "#5B57D6", { roughness: 0.7 });
      g.add(ball(THREE, cloth, [0, -0.02, 0], [0.44, 0.34, 0.44], 32));
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.34, 0.03, 24, 1, false, -0.6, 1.2), cloth); brim.position.set(0, -0.14, 0.28); brim.scale.set(1.15, 1, 1.4); g.add(brim);
      g.add(ball(THREE, accent, [0, 0.31, 0], 0.045, 10));
      const panel = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.16, 0.04), accent); panel.position.set(0, 0.06, 0.4); g.add(panel);
      return g;
    },
    bucket(THREE) {
      const g = new THREE.Group(), cloth = matte(THREE, "#6B7A4E", { roughness: 0.9 });   // olive
      const top = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.4, 0.34, 28), cloth); top.position.y = 0.06; g.add(top);
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.56, 0.62, 0.05, 32), cloth); brim.position.y = -0.13; g.add(brim);
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.405, 0.405, 0.05, 28), matte(THREE, "#3A3F2B", { roughness: 0.9 })); band.position.y = -0.08; g.add(band);
      return g;
    },
    flowers(THREE) {
      const g = new THREE.Group();
      const vine = new THREE.Mesh(new THREE.TorusGeometry(0.42, 0.045, 10, 40), matte(THREE, "#5FA85A", { roughness: 0.8 })); vine.rotation.x = Math.PI / 2; vine.position.y = -0.14; g.add(vine);
      const petals = ["#FF8FB1", "#FFFFFF", "#C9A7FF"];
      for (let i = 0; i < 6; i++) {
        const a = i / 6 * Math.PI * 2, f = new THREE.Group();
        f.position.set(Math.sin(a) * 0.42, -0.1, Math.cos(a) * 0.42);
        f.add(ball(THREE, matte(THREE, petals[i % 3], { roughness: 0.6 }), [0, 0, 0], [0.11, 0.045, 0.11], 14));
        f.add(ball(THREE, matte(THREE, "#FFD166", { roughness: 0.6 }), [0, 0.03, 0], 0.045, 10));
        g.add(f);
      }
      return g;
    },
  };
  // ---- hat maker ----------------------------------------------------------
  // Shapes the owner colours in the app. Rendered with id colours (main red, trim green, the sticker spot blue)
  // so the app can paint any colours onto the finished frames and put a sticker on the spot. The web hands
  // real colours in: wear(THREE, pet, sp, "maker:cap:1E90FF:FFFFFF").
  function makerMats(THREE, c) {
    c = c || {};
    const hex = (v, d) => (v ? "#" + String(v).replace("#", "") : d);
    // Phong, not Standard: no Fresnel, so a brim seen edge-on keeps its colour instead of going white.
    // Id colours at half strength so nothing clips without tone mapping; real colours look fine either way.
    const ph = (hexv) => new THREE.MeshPhongMaterial({ color: new THREE.Color(hexv), specular: new THREE.Color("#1A1A1A"), shininess: 22, side: THREE.DoubleSide });
    return { main: ph(hex(c.main, "#800000")), trim: ph(hex(c.trim, "#008000")), badge: ph(hex(c.badge, "#000080")) };
  }
  function badge(THREE, mat, g, pos, r, tiltX) {   // the flat spot the sticker goes on, facing forward
    const d = new THREE.Mesh(new THREE.CircleGeometry(r, 24), mat);
    d.position.set(pos[0], pos[1], pos[2]); d.rotation.x = tiltX || 0; d.castShadow = false; d.receiveShadow = true; g.add(d);
    return d;
  }
  const MAKER = {
    cap(THREE, c) {   // a snapback
      const M = makerMats(THREE, c), g = new THREE.Group();
      g.add(ball(THREE, M.main, [0, -0.02, 0], [0.44, 0.34, 0.44], 32));
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.34, 0.03, 24, 1, false, -0.6, 1.2), M.trim); brim.position.set(0, -0.14, 0.28); brim.scale.set(1.15, 1, 1.4); g.add(brim);
      g.add(ball(THREE, M.trim, [0, 0.31, 0], 0.045, 10));
      const panel = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.16, 0.04), M.trim); panel.position.set(0, 0.06, 0.4); g.add(panel);
      badge(THREE, M.badge, g, [0, 0.06, 0.425], 0.085, 0);
      return g;
    },
    beanie(THREE, c) {
      const M = makerMats(THREE, c), g = new THREE.Group();
      g.add(ball(THREE, M.main, [0, 0.03, 0], [0.4, 0.8 * 0.4, 0.4], 32));
      const brim = new THREE.Mesh(new THREE.TorusGeometry(0.385, 0.055, 12, 40), M.trim); brim.rotation.x = Math.PI / 2; brim.position.y = -0.12; g.add(brim);
      g.add(ball(THREE, M.trim, [0, 0.4, 0], 0.1, 20));
      badge(THREE, M.badge, g, [0, -0.12, 0.443], 0.045, 0);
      return g;
    },
    bucket(THREE, c) {
      const M = makerMats(THREE, c), g = new THREE.Group();
      const top = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.4, 0.34, 28), M.main); top.position.y = 0.06; g.add(top);
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.56, 0.62, 0.05, 32), M.main); brim.position.y = -0.13; g.add(brim);
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.405, 0.405, 0.05, 28), M.trim); band.position.y = -0.08; g.add(band);
      badge(THREE, M.badge, g, [0, 0.1, 0.368], 0.09, -0.17);
      return g;
    },
    tophat(THREE, c) {
      const M = makerMats(THREE, c), g = new THREE.Group();
      const crown = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.33, 0.5, 32), M.main); crown.position.y = 0.16; g.add(crown);
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.58, 0.58, 0.04, 32), M.main); brim.position.y = -0.1; g.add(brim);
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.345, 0.345, 0.1, 32), M.trim); band.position.y = -0.03; g.add(band);
      badge(THREE, M.badge, g, [0, -0.03, 0.352], 0.055, 0);
      return g;
    },
    cowboy(THREE, c) {   // a ranch hat: a dished brim that curls up all round
      const M = makerMats(THREE, c), g = new THREE.Group();
      g.add(ball(THREE, M.main, [0, 0.0, 0], [0.4, 0.36, 0.42], 32));
      const prof = [[0.36, -0.12], [0.52, -0.135], [0.64, -0.11], [0.72, -0.05], [0.735, 0.0], [0.7, -0.01], [0.63, -0.07], [0.52, -0.1], [0.36, -0.085]].map(([r, y]) => new THREE.Vector2(r, y));
      const brim = new THREE.Mesh(new THREE.LatheGeometry(prof, 40), M.main); brim.scale.z = 0.86; brim.castShadow = true; brim.receiveShadow = true; g.add(brim);
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.405, 0.415, 0.07, 32), M.trim); band.position.y = -0.09; band.scale.z = 1.04; g.add(band);
      badge(THREE, M.badge, g, [0, 0.05, 0.415], 0.08, -0.2);
      return g;
    },
    visor(THREE, c) {
      const M = makerMats(THREE, c), g = new THREE.Group();
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.57, 0.575, 0.1, 40, 1, true), M.main); band.position.y = -0.05; g.add(band);
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.575, 0.8, 0.14, 32, 1, true, -0.8, 1.6), M.trim); brim.position.y = -0.165; g.add(brim);   // a curved brim sloping down from the band
      badge(THREE, M.badge, g, [0, -0.03, 0.58], 0.042, 0);
      return g;
    },
  };
  const MAKER_SHAPES = Object.keys(MAKER);
  function makerBuild(id) {   // "maker:cap" or "maker:cap:MAIN:TRIM[:BADGE]" -> a builder, or null
    if (typeof id !== "string") return null;
    const p = id.split(":");
    if (p[0] !== "maker" || !MAKER[p[1]]) return null;
    return (THREE) => MAKER[p[1]](THREE, { main: p[2], trim: p[3], badge: p[4] || p[2] });
  }
  // Face items sit in front of the eyes; built for a head radius of 0.55 with the eyes at x = +-0.17.
  const FACE = {
    glasses(THREE) {
      const g = new THREE.Group(), frame = matte(THREE, "#2B2540", { roughness: 0.4 });
      for (const sx of [-1, 1]) {
        const ring = new THREE.Mesh(new THREE.TorusGeometry(0.135, 0.017, 10, 32), frame); ring.position.set(sx * 0.17, 0, 0); g.add(ring);
        const arm = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.02, 0.42), frame); arm.position.set(sx * 0.31, 0.03, -0.2); g.add(arm);
      }
      const bridge = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.02, 0.02), frame); bridge.position.set(0, 0.02, 0); g.add(bridge);
      return g;
    },
    sunglasses(THREE) {
      const g = new THREE.Group(), frame = matte(THREE, "#1E1B24", { roughness: 0.35 });
      const lens = new THREE.MeshStandardMaterial({ color: new THREE.Color("#171222"), roughness: 0.15, metalness: 0.3, transparent: true, opacity: 0.96 });
      for (const sx of [-1, 1]) {
        const l = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.15, 0.03, 28), lens); l.rotation.x = Math.PI / 2; l.position.set(sx * 0.17, 0, 0); g.add(l);
        const rim = new THREE.Mesh(new THREE.TorusGeometry(0.15, 0.022, 10, 32), frame); rim.position.set(sx * 0.17, 0, 0); g.add(rim);
        const arm = new THREE.Mesh(new THREE.BoxGeometry(0.024, 0.024, 0.42), frame); arm.position.set(sx * 0.33, 0.03, -0.2); g.add(arm);
      }
      const bridge = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.026, 0.026), frame); bridge.position.set(0, 0.03, 0); g.add(bridge);
      return g;
    },
  };
  // Neck items hang below the head; built for a head radius of 0.55, origin at the head centre.
  const NECK = {
    chain(THREE) {
      const g = new THREE.Group(), gold = new THREE.MeshStandardMaterial({ color: new THREE.Color("#F5C242"), roughness: 0.25, metalness: 0.7 });
      for (let i = 0; i < 14; i++) {
        const a = -Math.PI * 0.95 + i / 13 * Math.PI * 0.9;                 // an arc across the front
        const link = new THREE.Mesh(new THREE.TorusGeometry(0.04, 0.014, 8, 14), gold);
        link.position.set(Math.cos(a) * 0.5, -0.5 - Math.sin(-a) * 0.0 - Math.abs(Math.sin(a + Math.PI / 2)) * 0.12, Math.sin(a) * -0.5 * -1);
        link.position.set(Math.cos(a) * 0.5, -0.46 - (1 - Math.abs(Math.cos(a))) * 0.16, Math.abs(Math.sin(a)) * 0.5);
        link.rotation.y = a; g.add(link);
      }
      const pendant = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.03, 24), gold); pendant.rotation.x = Math.PI / 2; pendant.position.set(0, -0.66, 0.5); g.add(pendant);
      return g;
    },
  };
  // Body items sit over the body; built for the body ball at (0, -0.2, 0) scaled about 0.48 x 0.44 x 0.46.
  const BODY = {
    hoodie(THREE) {
      const g = new THREE.Group(), cloth = matte(THREE, "#2B2540", { roughness: 0.95 });
      g.add(ball(THREE, cloth, [0, -0.2, 0], [0.5, 0.45, 0.48], 32));                          // over the body
      const pocket = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.14, 0.05), matte(THREE, "#3A3550", { roughness: 0.95 })); pocket.position.set(0, -0.36, 0.44); g.add(pocket);
      const hood = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.11, 12, 28, Math.PI), cloth); hood.position.set(0, 0.2, -0.22); hood.rotation.x = -0.9; g.add(hood);   // behind the neck
      for (const sx of [-1, 1]) { const str = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.22, 6), matte(THREE, "#E8DFF3")); str.position.set(sx * 0.1, 0.06, 0.44); g.add(str); }
      return g;
    },
  };
  // Ear items wrap the head from the side; built for a head radius of 0.55, origin at the head centre.
  const EARS = {
    headphones(THREE) {
      const g = new THREE.Group(), shell = matte(THREE, "#5B57D6", { roughness: 0.45 }), pad = matte(THREE, "#2B2540", { roughness: 0.8 });
      const band = new THREE.Mesh(new THREE.TorusGeometry(0.6, 0.045, 10, 40, Math.PI), shell); band.position.y = -0.04; g.add(band);   // over the top, ear to ear
      for (const sx of [-1, 1]) {
        const cup = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.17, 0.1, 28), shell); cup.rotation.z = Math.PI / 2; cup.position.set(sx * 0.6, -0.06, 0.04); g.add(cup);
        const cushion = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.04, 28), pad); cushion.rotation.z = Math.PI / 2; cushion.position.set(sx * 0.54, -0.06, 0.04); g.add(cushion);
      }
      return g;
    },
  };
  // ---- the wave: more of everything ------------------------------------------
  Object.assign(HATS, {
    wizard(THREE) {   // a tall cone with a wide brim, a bend at the tip, and stars
      const g = new THREE.Group(), cloth = matte(THREE, "#3B2A7A", { roughness: 0.8 }), gold = matte(THREE, "#F5C242", { roughness: 0.4 });
      const cone = new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.9, 32), cloth); cone.position.y = 0.4; cone.rotation.z = -0.12; g.add(cone);
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.04, 40), cloth); brim.position.y = -0.03; g.add(brim);
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.36, 0.08, 32), gold); band.position.y = 0.02; g.add(band);
      for (const [x, y, z] of [[0.12, 0.35, 0.28], [-0.14, 0.55, 0.2], [0.05, 0.72, 0.14]]) {
        const s = new THREE.Mesh(new THREE.OctahedronGeometry(0.045), gold); s.position.set(x, y, z); g.add(s);
      }
      return g;
    },
    pirate(THREE) {   // a tricorn with a skull
      const g = new THREE.Group(), felt = matte(THREE, "#1E1B24", { roughness: 0.85 }), bone = matte(THREE, "#F4F1EA", { roughness: 0.6 });
      g.add(ball(THREE, felt, [0, -0.02, 0], [0.42, 0.3, 0.42], 32));
      const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.035, 40), felt); brim.position.y = -0.1; g.add(brim);
      for (const a of [0, 2.1, -2.1]) {   // three sides turned up
        const flap = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.22, 0.035), felt);
        flap.position.set(Math.sin(a) * 0.5, 0.0, Math.cos(a) * 0.5); flap.rotation.y = a; flap.rotation.x = 0.35; g.add(flap);
      }
      g.add(ball(THREE, bone, [0, 0.05, 0.47], [0.07, 0.08, 0.04], 16));
      for (const sx of [-1, 1]) g.add(ball(THREE, felt, [sx * 0.025, 0.06, 0.505], 0.018, 8));
      return g;
    },
    viking(THREE) {   // a helmet with horns
      const g = new THREE.Group(), steel = new THREE.MeshStandardMaterial({ color: new THREE.Color("#B8BCC8"), roughness: 0.35, metalness: 0.7 }), horn = matte(THREE, "#F4F1EA", { roughness: 0.5 });
      g.add(ball(THREE, steel, [0, -0.06, 0], [0.5, 0.42, 0.5], 32));
      const rim = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.035, 10, 40), steel); rim.rotation.x = Math.PI / 2; rim.position.y = -0.14; g.add(rim);
      const nose = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.24, 0.03), steel); nose.position.set(0, -0.24, 0.5); g.add(nose);
      for (const sx of [-1, 1]) {
        const h = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.45, 16), horn); h.position.set(sx * 0.5, 0.15, 0); h.rotation.z = -sx * 0.7; g.add(h);
      }
      return g;
    },
    halo(THREE) {
      const g = new THREE.Group(), gold = new THREE.MeshStandardMaterial({ color: new THREE.Color("#FFD966"), roughness: 0.2, metalness: 0.6, emissive: new THREE.Color("#FFB300"), emissiveIntensity: 0.35 });
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.04, 12, 40), gold); ring.rotation.x = Math.PI / 2; ring.position.y = 0.28; ring.rotation.z = 0.15; g.add(ring);
      return g;
    },
    devil(THREE) {
      const g = new THREE.Group(), red = matte(THREE, "#D63B3B", { roughness: 0.4 });
      for (const sx of [-1, 1]) {
        const h = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.32, 16), red); h.position.set(sx * 0.28, 0.06, 0.05); h.rotation.z = -sx * 0.45; g.add(h);
      }
      return g;
    },
    catears(THREE) {
      const g = new THREE.Group(), fur = matte(THREE, "#2B2540", { roughness: 0.9 }), inner = matte(THREE, "#F58EA6", { roughness: 0.8 });
      for (const sx of [-1, 1]) {
        const ear = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.3, 4), fur); ear.position.set(sx * 0.3, 0.05, 0); ear.rotation.y = Math.PI / 4; ear.rotation.z = -sx * 0.35; g.add(ear);
        const pink = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.2, 4), inner); pink.position.set(sx * 0.3, 0.03, 0.05); pink.rotation.y = Math.PI / 4; pink.rotation.z = -sx * 0.35; g.add(pink);
      }
      const band = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.03, 8, 40, Math.PI), fur); band.position.y = -0.1; g.add(band);
      return g;
    },
    bunny(THREE) {
      const g = new THREE.Group(), fur = matte(THREE, "#F4F1EA", { roughness: 0.9 }), inner = matte(THREE, "#F58EA6", { roughness: 0.8 });
      for (const sx of [-1, 1]) {
        g.add(ball(THREE, fur, [sx * 0.2, 0.42, 0], [0.11, 0.42, 0.07], 20));
        g.add(ball(THREE, inner, [sx * 0.2, 0.44, 0.04], [0.06, 0.3, 0.04], 16));
      }
      const band = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.03, 8, 40, Math.PI), fur); band.position.y = -0.1; g.add(band);
      return g;
    },
    tiara(THREE) {
      const g = new THREE.Group(), gold = new THREE.MeshStandardMaterial({ color: new THREE.Color("#F5C242"), roughness: 0.25, metalness: 0.7 }), gem = matte(THREE, "#48B7E8", { roughness: 0.2 });
      const arc = new THREE.Mesh(new THREE.TorusGeometry(0.46, 0.03, 8, 40, Math.PI * 0.9), gold); arc.rotation.x = Math.PI / 2; arc.rotation.z = Math.PI * 0.05; arc.position.y = -0.06; g.add(arc);
      for (let i = -2; i <= 2; i++) {
        const a = i * 0.32, h = 0.1 + (2 - Math.abs(i)) * 0.05;
        const pt = new THREE.Mesh(new THREE.ConeGeometry(0.035, h, 8), gold); pt.position.set(Math.sin(a) * 0.46, -0.06 + h / 2, Math.cos(a) * 0.46); g.add(pt);
      }
      g.add(ball(THREE, gem, [0, 0.06, 0.47], 0.05, 12));
      return g;
    },
    sweatband(THREE) {
      const g = new THREE.Group(), terry = matte(THREE, "#F4F1EA", { roughness: 1.0 }), stripe = matte(THREE, "#D63B3B", { roughness: 1.0 });
      const band = new THREE.Mesh(new THREE.CylinderGeometry(0.545, 0.55, 0.13, 40, 1, true), terry); band.position.y = -0.36; g.add(band);
      const s = new THREE.Mesh(new THREE.CylinderGeometry(0.552, 0.555, 0.03, 40, 1, true), stripe); s.position.y = -0.36; g.add(s);
      return g;
    },
  });
  Object.assign(FACE, {
    eyepatch(THREE) {
      const g = new THREE.Group(), dark = matte(THREE, "#1E1B24", { roughness: 0.6 });
      const patch = new THREE.Mesh(new THREE.SphereGeometry(1, 20, 12), dark); patch.scale.set(0.15, 0.13, 0.05); patch.position.set(0.17, 0.0, 0.1); g.add(patch);
      const strap = new THREE.Mesh(new THREE.TorusGeometry(0.52, 0.014, 6, 48), dark); strap.position.set(0, 0.06, -0.48); strap.rotation.x = Math.PI / 2 - 0.25; g.add(strap);   // around the head, a little tilted
      return g;
    },
    monocle(THREE) {
      const g = new THREE.Group(), gold = new THREE.MeshStandardMaterial({ color: new THREE.Color("#F5C242"), roughness: 0.3, metalness: 0.7 });
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.13, 0.016, 10, 32), gold); ring.position.set(0.17, 0, 0); g.add(ring);
      for (let i = 0; i < 6; i++) { const l = new THREE.Mesh(new THREE.TorusGeometry(0.02, 0.007, 6, 10), gold); l.position.set(0.3 + i * 0.02, -0.12 - i * 0.05, 0); g.add(l); }
      return g;
    },
    glasses3d(THREE) {
      const g = new THREE.Group(), card = matte(THREE, "#F4F1EA", { roughness: 0.8 });
      const frame = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.2, 0.02), card); frame.position.set(0, 0.01, 0); g.add(frame);
      const red = new THREE.MeshStandardMaterial({ color: new THREE.Color("#E23B3B"), transparent: true, opacity: 0.75, roughness: 0.2 });
      const blue = new THREE.MeshStandardMaterial({ color: new THREE.Color("#2F7BE2"), transparent: true, opacity: 0.75, roughness: 0.2 });
      for (const [sx, m] of [[-1, red], [1, blue]]) { const lens = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.14, 0.03), m); lens.position.set(sx * 0.17, 0.01, 0.005); g.add(lens); }
      for (const sx of [-1, 1]) { const arm = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.03, 0.42), card); arm.position.set(sx * 0.31, 0.03, -0.2); g.add(arm); }
      return g;
    },
    moustache(THREE) {
      const g = new THREE.Group(), hair = matte(THREE, "#2B2540", { roughness: 0.9 });
      for (const sx of [-1, 1]) {
        const half = ball(THREE, hair, [sx * 0.11, -0.21, 0.08], [0.13, 0.045, 0.03], 16); half.rotation.z = sx * 0.35; g.add(half);
        g.add(ball(THREE, hair, [sx * 0.22, -0.16, 0.07], 0.035, 10));
      }
      g.add(ball(THREE, hair, [0, -0.2, 0.09], [0.05, 0.04, 0.03], 12));
      return g;
    },
  });
  Object.assign(NECK, {
    bowtie(THREE) {
      const g = new THREE.Group(), silk = matte(THREE, "#D63B3B", { roughness: 0.4 });
      for (const sx of [-1, 1]) { const w = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.2, 3), silk); w.rotation.z = sx * Math.PI / 2; w.position.set(sx * 0.1, -0.56, 0.47); g.add(w); }
      g.add(ball(THREE, silk, [0, -0.56, 0.48], [0.04, 0.04, 0.03], 10));
      return g;
    },
    scarf(THREE) {
      const g = new THREE.Group(), wool = matte(THREE, "#D63B3B", { roughness: 1.0 }), stripe = matte(THREE, "#F4F1EA", { roughness: 1.0 });
      const loop = new THREE.Mesh(new THREE.TorusGeometry(0.47, 0.09, 12, 40), wool); loop.rotation.x = Math.PI / 2; loop.position.y = -0.55; g.add(loop);
      const tail = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.42, 0.06), wool); tail.position.set(0.22, -0.8, 0.45); tail.rotation.z = 0.15; g.add(tail);
      for (const y of [-0.7, -0.85, -1.0]) { const s = new THREE.Mesh(new THREE.BoxGeometry(0.165, 0.04, 0.065), stripe); s.position.set(0.22 + (y + 0.8) * -0.15, y, 0.45); s.rotation.z = 0.15; g.add(s); }
      return g;
    },
    bandana(THREE) {
      const g = new THREE.Group(), cloth = matte(THREE, "#3B5BDB", { roughness: 0.9 });
      const tri = new THREE.Mesh(new THREE.ConeGeometry(0.3, 0.36, 3), cloth); tri.rotation.z = Math.PI; tri.rotation.y = Math.PI / 6; tri.position.set(0, -0.72, 0.42); tri.scale.z = 0.25; g.add(tri);
      const knot = new THREE.Mesh(new THREE.TorusGeometry(0.47, 0.035, 8, 40), cloth); knot.rotation.x = Math.PI / 2; knot.position.y = -0.54; g.add(knot);
      return g;
    },
  });
  Object.assign(BODY, {
    tshirt(THREE) {
      const g = new THREE.Group(), cotton = matte(THREE, "#F4F1EA", { roughness: 0.95 });
      g.add(ball(THREE, cotton, [0, -0.2, 0], [0.49, 0.42, 0.47], 32));
      const print = new THREE.Mesh(new THREE.CircleGeometry(0.12, 24), matte(THREE, "#2FB3A3", { roughness: 0.9 })); print.position.set(0, -0.14, 0.47); g.add(print);
      return g;
    },
    jersey(THREE) {
      const g = new THREE.Group(), mesh = matte(THREE, "#3B5BDB", { roughness: 0.9 }), white = matte(THREE, "#F4F1EA", { roughness: 0.9 });
      g.add(ball(THREE, mesh, [0, -0.2, 0], [0.49, 0.42, 0.47], 32));
      for (const sx of [-1, 1]) { const s = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.3, 0.02), white); s.position.set(sx * 0.2, -0.12, 0.44); s.rotation.z = -sx * 0.5; g.add(s); }
      const num = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.16, 0.02), white); num.position.set(0, -0.2, 0.47); g.add(num);
      const hole = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.08, 0.025), mesh); hole.position.set(0, -0.2, 0.475); g.add(hole);
      return g;
    },
    suit(THREE) {
      const g = new THREE.Group(), wool = matte(THREE, "#23213B", { roughness: 0.85 }), shirt = matte(THREE, "#F4F1EA", { roughness: 0.8 }), tie = matte(THREE, "#D63B3B", { roughness: 0.5 });
      g.add(ball(THREE, wool, [0, -0.2, 0], [0.49, 0.42, 0.47], 32));
      const v = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.3, 3), shirt); v.rotation.z = Math.PI; v.rotation.y = Math.PI / 6; v.position.set(0, -0.12, 0.46); v.scale.z = 0.2; g.add(v);
      const t = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.22, 0.02), tie); t.position.set(0, -0.16, 0.48); g.add(t);
      for (const sx of [-1, 1]) { const lapel = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.26, 0.02), wool); lapel.position.set(sx * 0.11, -0.1, 0.475); lapel.rotation.z = sx * 0.4; g.add(lapel); }
      return g;
    },
    sweater(THREE) {
      const g = new THREE.Group(), knit = matte(THREE, "#6B7A4E", { roughness: 1.0 }), cream = matte(THREE, "#F4F1EA", { roughness: 1.0 });
      g.add(ball(THREE, knit, [0, -0.2, 0], [0.5, 0.43, 0.48], 32));
      for (let i = 0; i < 3; i++) { const z = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.025, 0.02), cream); z.position.set(0, -0.08 - i * 0.1, 0.47 - i * 0.01); g.add(z); }
      const collar = new THREE.Mesh(new THREE.TorusGeometry(0.2, 0.05, 10, 28), knit); collar.rotation.x = Math.PI / 2; collar.position.set(0, 0.22, 0); g.add(collar);
      return g;
    },
  });
  // Back items hang off the body behind it; same space as body items.
  const BACK = {
    angelwings(THREE) {
      const g = new THREE.Group(), feather = matte(THREE, "#FFFFFF", { roughness: 0.7, side: THREE.DoubleSide });
      for (const sx of [-1, 1]) {
        for (const [dx, dy, w, h, rot] of [[0.62, 0.12, 0.42, 0.24, 0.55], [0.74, -0.08, 0.36, 0.2, 0.3], [0.8, -0.28, 0.3, 0.16, 0.1]]) {
          const f = new THREE.Mesh(new THREE.SphereGeometry(1, 20, 12), feather); f.scale.set(w, h, 0.05); f.position.set(sx * dx, dy - 0.05, -0.3); f.rotation.z = sx * rot; g.add(f);
        }
      }
      return g;
    },
    batwings(THREE) {
      const g = new THREE.Group(), skin = matte(THREE, "#2B2540", { roughness: 0.8, side: THREE.DoubleSide });
      for (const sx of [-1, 1]) {
        const shape = new THREE.Shape(); shape.moveTo(0, 0); shape.lineTo(0.35, 0.25); shape.lineTo(0.62, 0.05); shape.lineTo(0.5, -0.15); shape.lineTo(0.6, -0.35); shape.lineTo(0.3, -0.3); shape.lineTo(0.15, -0.45); shape.lineTo(0, -0.2); shape.lineTo(0, 0);
        const w = new THREE.Mesh(new THREE.ShapeGeometry(shape), skin); w.position.set(sx * 0.3, 0.05, -0.3); w.scale.set(sx * 1.3, 1.3, 1); g.add(w);
      }
      return g;
    },
    backpack(THREE) {
      const g = new THREE.Group(), nylon = matte(THREE, "#D63B3B", { roughness: 0.9 }), dark = matte(THREE, "#23213B", { roughness: 0.9 });
      const bag = new THREE.Mesh(new THREE.BoxGeometry(0.66, 0.46, 0.26), nylon); bag.position.set(0, -0.2, -0.5); g.add(bag);
      const pocket = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.2, 0.06), dark); pocket.position.set(0, -0.3, -0.65); g.add(pocket);
      const lid = new THREE.Mesh(new THREE.BoxGeometry(0.68, 0.1, 0.28), dark); lid.position.set(0, 0.06, -0.5); g.add(lid);
      for (const sx of [-1, 1]) {   // straps come over the shoulders and down the chest
        const strap = new THREE.Mesh(new THREE.TorusGeometry(0.42, 0.03, 8, 30, Math.PI * 0.9), dark); strap.position.set(sx * 0.24, -0.16, 0.02); strap.rotation.y = Math.PI / 2; strap.rotation.z = Math.PI * 0.55; g.add(strap);
      }
      return g;
    },
    jetpack(THREE) {
      const g = new THREE.Group(), steel = new THREE.MeshStandardMaterial({ color: new THREE.Color("#B8BCC8"), roughness: 0.35, metalness: 0.7 }), flame = matte(THREE, "#FF9F1C", { roughness: 0.5 });
      for (const sx of [-1, 1]) {
        const tank = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.46, 20), steel); tank.position.set(sx * 0.4, -0.22, -0.36); g.add(tank);
        const cap = new THREE.Mesh(new THREE.SphereGeometry(0.13, 20, 12), steel); cap.position.set(sx * 0.4, 0.01, -0.36); g.add(cap);
        const fire = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.3, 12), flame); fire.rotation.x = Math.PI; fire.position.set(sx * 0.4, -0.6, -0.36); g.add(fire);
      }
      const plate = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.3, 0.06), steel); plate.position.set(0, -0.2, -0.42); g.add(plate);
      return g;
    },
  };
  const SHELF_OF = (id) => (HATS[id] || makerBuild(id) ? "hat" : FACE[id] ? "face" : EARS[id] ? "ears" : NECK[id] ? "neck" : BODY[id] ? "body" : BACK[id] ? "back" : null);

  function makeOutfit(THREE, id, sp) {
    const shelf = SHELF_OF(id);
    if (!shelf) return null;
    const st = STYLE[sp.style] || STYLE.cute, headR = st.headR;
    const k = headR / 0.55;
    const g = new THREE.Group(); g.userData.outfit = id; g.userData.shelf = shelf;
    let item;
    if (shelf === "hat") {
      const top = headR * (sp.id === "ears" ? 0.96 : 1);
      const seat = HAT_SEAT[sp.id] || { tilt: 0, y: 0 };
      const build = HATS[id] || makerBuild(id);
      if (!build) return null;
      item = build(THREE); item.position.y = top + seat.y;
      if (sp.id === "ears" && id.startsWith("maker:visor")) { item.scale.x *= 1.1; item.scale.z *= 1.05; }   // Pink's head is wider
      g.rotation.z = seat.tilt;   // turning around the head centre keeps it on the head
    } else if (shelf === "face") {
      item = FACE[id](THREE); item.position.set(0, st.faceY + st.eyeY, headR * 0.9 + 0.05);
    } else if (shelf === "neck") {
      item = NECK[id](THREE); item.position.set(0, -headR + 0.55 - 0.02, 0);                   // hangs from the head's underside
      if (sp.id === "ears") item.scale.x = 1.1;
    } else if (shelf === "body" || shelf === "back") {
      item = (shelf === "body" ? BODY : BACK)[id](THREE);
      const wide = sp.id === "ears"; item.scale.set(wide ? 1.15 : 1, st.bodyH / 0.44, wide ? 1.08 : 1);
      const g2 = new THREE.Group(); g2.userData.outfit = id; g2.userData.shelf = shelf; g2.add(item);
      item.traverse((m) => { if (m.isMesh) { m.castShadow = true; m.receiveShadow = true; } });
      return g2;                                                                                  // body items hang off the body, not the head
    } else {
      item = EARS[id](THREE); item.position.set(0, sp.id === "ears" ? -0.08 : 0, 0);   // under Pink's ears
      if (sp.id === "ears") item.scale.x = 1.07;
    }
    item.scale.multiplyScalar(k);
    item.traverse((m) => { if (m.isMesh) { m.castShadow = true; m.receiveShadow = true; } });
    g.add(item);
    return g;
  }
  // One item per shelf. wear(pet, sp, "beanie") swaps the hat and leaves the glasses on; wear(pet, sp, "", "hat") takes the hat off.
  function wear(THREE, pet, sp, id, shelf) {
    const head = pet.userData.parts.head, body = pet.userData.parts.bodyRoot;
    const target = shelf || SHELF_OF(id) || "hat";
    for (const host of [head, body]) for (const c of host.children.slice()) if (c.userData.outfit && c.userData.shelf === target) host.remove(c);
    if (!id) return null;
    const g = makeOutfit(THREE, id, sp);
    if (g) (target === "body" || target === "back" ? body : head).add(g);
    return g;
  }

  function setMood(g, mood) {
    const p = g.userData.parts, st = STYLE[g.userData.style] || STYLE.cute;
    g.userData.mood = mood;
    p.eyes.visible = mood !== "sleepy"; p.sleepyLines.visible = mood === "sleepy";
    p.smile.visible = mood === "happy"; p.open.visible = mood === "happy" && !st.lids; p.frown.visible = mood === "sulky";
    p.oh.visible = (mood === "surprised" || mood === "sleepy"); p.flat.visible = false;
    if (st.lids && mood === "happy") { p.smile.visible = true; }
    p.blush.visible = mood !== "sulky";
    const a = st.browAmp;
    const set = (b, sx, y, rz) => { b.position.y = y; b.rotation.z = sx * rz * a; };
    const [L, R] = p.brows;
    if (mood === "happy")     { set(L, -1, st.browY, 0.18);         set(R, 1, st.browY, 0.18); }
    if (mood === "surprised") { set(L, -1, st.browY + 0.05, 0.05);  set(R, 1, st.browY + 0.05, 0.05); }
    if (mood === "sleepy")    { set(L, -1, st.browY - 0.03, 0.1);   set(R, 1, st.browY - 0.03, 0.1); }
    if (mood === "sulky")     { set(L, -1, st.browY - 0.03, -0.35); set(R, 1, st.browY - 0.03, -0.35); }
    const ey = mood === "surprised" ? 1.18 : (mood === "sulky" ? 0.78 : 1);
    for (const e of p.eyeParts) e.scale.set(1, ey, 1);
    if (mood === "surprised") p.oh.scale.set(0.04, 0.05, 0.03); else p.oh.scale.set(0.03, 0.028, 0.025);
    g.userData.tilt = mood === "sulky" ? { x: 0.08, y: 0.55 } : (mood === "sleepy" ? { x: 0.2, y: 0 } : { x: 0, y: 0 });
  }

  function setPose(g, pose) {
    const p = g.userData.parts;
    g.rotation.z = 0; g.position.y = 0; g.scale.set(1, 1, 1);
    for (const f of p.feet) { f.position.set(Math.sign(f.position.x) * 0.16, -0.66, 0.08); f.rotation.set(0, 0, 0); }
    if (p.arms) for (const a of p.arms) { a.position.set(Math.sign(a.position.x) * g.userData.armX, -0.16, 0.1); a.scale.set(0.12, 0.13, 0.12); a.rotation.z = 0; }
    if (p.head) { p.head.rotation.z = 0; p.head.rotation.x = 0; }
    if (p.props) for (const k in p.props) p.props[k].visible = false;
    if (pose === "blink") for (const e of p.eyeParts) e.scale.y *= 0.1;
    const sitting = ["sit", "study", "work", "game", "eat1", "eat2"].includes(pose);
    if (sitting) {   // feet out front, soles showing, a little settled
      for (const f of p.feet) { f.position.set(Math.sign(f.position.x) * 0.2, -0.56, 0.34); f.rotation.x = -0.9; }
      g.scale.set(1.04, 0.95, 1.04);
    }
    if (p.props) {
      if (pose === "study") { p.props.laptop.visible = true; p.props.book.visible = true; }
      if (pose === "work") { p.props.laptop.visible = true; p.props.mug.visible = true; }
      if (pose === "game") { p.props.pad.visible = true; for (const a of p.arms) { a.position.set(Math.sign(a.position.x) * 0.2, -0.3, 0.36); a.scale.set(0.11, 0.11, 0.11); } }
      if (pose === "eat1" || pose === "eat2") { p.props.bowl.visible = true; for (const a of p.arms) { a.position.set(Math.sign(a.position.x) * 0.24, -0.4, 0.34); } }
      if (pose === "eat2") { p.smile.visible = false; p.open.visible = false; p.oh.visible = true; p.oh.scale.set(0.04, 0.045, 0.03); }   // mid-chew
    }
    if (pose === "lie") { g.rotation.z = -1.35; g.position.y = -0.18; }   // on its side, head to the right
    if (pose === "dab") {
      const [l, r] = p.arms;
      r.position.set(g.userData.armX + 0.22, 0.02, 0.2); r.scale.set(0.11, 0.17, 0.11); r.rotation.z = -0.6;      // out and up
      l.position.set(0.02, 0.28, 0.42); l.scale.set(0.11, 0.16, 0.11); l.rotation.z = 0.9;                       // across the face
      p.head.rotation.z = -0.28; p.head.rotation.x = 0.18;
    }
    if (pose === "flex") {
      for (const a of p.arms) { a.position.set(Math.sign(a.position.x) * (g.userData.armX + 0.12), 0.22, 0.12); a.scale.set(0.12, 0.19, 0.12); a.rotation.z = Math.sign(a.position.x) * 0.35; }
      g.scale.set(1.02, 1.04, 1.02);
    }
    if (pose === "wave1" || pose === "wave2") {
      const a = p.arms[1], up = pose === "wave2";
      a.position.set(g.userData.armX + (up ? 0.12 : 0.05), up ? 0.2 : 0.08, 0.14); a.scale.set(0.12, 0.18, 0.12);
    }
    if (pose === "walk1") { p.feet[0].position.z = 0.22; p.feet[1].position.z = -0.06; g.rotation.z = 0.05; g.position.y = 0.03; }
    if (pose === "walk2") { p.feet[0].position.z = -0.06; p.feet[1].position.z = 0.22; g.rotation.z = -0.05; g.position.y = 0.03; }
    if (pose === "squash") { g.scale.set(1.08, 0.9, 1.08); }
    if (pose === "stretch") { g.scale.set(0.95, 1.08, 0.95); g.position.y = 0.06; }
  }

  // Shared stage lighting so the demo and the sprite renderer match.
  function addLights(THREE, scene, opts) {
    const o = opts || {};
    scene.add(new THREE.HemisphereLight(0xffffff, 0x2a1840, 0.32));
    const key = new THREE.DirectionalLight(0xfff4e6, 1.45); key.position.set(3, 5.5, 4); key.castShadow = true;
    const s = o.shadowSize || 6;
    key.shadow.mapSize.set(o.shadowMap || 2048, o.shadowMap || 2048);
    key.shadow.camera.left = -s; key.shadow.camera.right = s; key.shadow.camera.top = s; key.shadow.camera.bottom = -s;
    key.shadow.camera.near = 1; key.shadow.camera.far = 20; key.shadow.radius = 4;
    scene.add(key);
    const fill = new THREE.DirectionalLight(0xc8b6ff, 0.22); fill.position.set(-4, 2, 3); scene.add(fill);
    const rim = new THREE.DirectionalLight(0xff9ed6, 0.6); rim.position.set(0, 3, -5); scene.add(rim);
  }
  function setupRenderer(THREE, renderer) {
    renderer.outputEncoding = THREE.sRGBEncoding;
    renderer.toneMapping = THREE.ACESFilmicToneMapping; renderer.toneMappingExposure = 0.92;
    renderer.shadowMap.enabled = true; renderer.shadowMap.type = THREE.PCFSoftShadowMap;
  }

  root.Perchlings = { SPECIES, MOODS, POSES, STYLE, HATS, MAKER, MAKER_SHAPES, FACE, EARS, NECK, BODY, BACK, makePet, makeOutfit, wear, setMood, setPose, addLights, setupRenderer };
})(typeof window !== "undefined" ? window : globalThis);
