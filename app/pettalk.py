...
def facts(states, owner):
    ...
    f = {"owner": owner, "people": [], "likes": [], "dislikes": [], "favorites": [], "feelings": [], "events": [], "plans": [],
         ...
         "usual": None, "today_login": None, "reminders": [], "names": {}}
    all_logins, touches = [], {}
    for pid, st in states.items():
        ...
        for note in st.get("notes", []):
            ...
            m = re.search(r"\bi(?:'m| am| feel| felt| was| have been)\s+(?:so |very |really |a bit |a little )?" + FEELINGS + r"\b", text, re.I)
...
def conversation(f, group, rnd):
    ...
    V = o.v
    exchanges = []      # each: list of lines; seats are assigned when we assemble

    for rel, name, age in f["people"][-3:]:
        exchanges.append([f"{P} {rel} is called {name}.",
                          rnd.choice([f"{name}. Does {name} know about us?", f"I'd like to meet {name}.", f"Has {s} mentioned {name} since?"]),
    ...
    for feeling, age in f["feelings"][-2:]:
        ex = [f"{S} said {s} {V('was', 'were')} {feeling} {_when(age)}."]
    ...
    for event, age, until in f["events"][-2:]:
        ...
        if until is not None and until > 0:
            ...
            ex.append(rnd.choice(["How did it go?", f"Did {s} say how it went?", "Should we ask?"])); ex.append(rnd.choice([f"Ask {obj}.", "No idea.", f"{S} didn't say."]))
        else:
    ...
    if len(f["picks"]) >= 2:
        ...
        if f["picks"][a_] and f["picks"][b_]:
            exchanges.append([(a_, f"{S} picked {rnd.choice(f['picks'][a_])} for me."), (b_, f"{rnd.choice(f['picks'][b_])} for me."), (a_, rnd.choice(["Fair.", "Suits you.", "Hehe."]))])
    if len(f["adopted_days"]) >= 2:
        ...
        exchanges.append([(pids[0], first), (pids[1], second), (pids[0], rnd.choice(["You'll like it.", "Time flies.", "Welcome."]) if d1 > d2 else "Hehe.")])
    if not o.name:
        exchanges.append([f"We still don't know {p} name.", rnd.choice(["Someone should ask.", "There's a notebook for that.", "Maybe tomorrow."])])

    # up to six exchanges; seats rotate, except lines that are about a particular pet, which that pet says
    rnd.shuffle(exchanges)
    chosen = exchanges[:6] if seats > 2 else exchanges[:5]
    talk = []
